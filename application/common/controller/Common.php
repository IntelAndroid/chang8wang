...
//            think\Log::notice(json_encode(session("user")));
            if (empty($usertype)) {
                return json(array('code' => '2', 'msg' => '用户角色获取失败', 'data' => []));
            }

            /*if (session("user.status") == 6018004) { //此状态暂时不用了
                return json(array('code' => '3', 'msg' => '用户角色获取失败', 'data' => ['url' => "{:url('/user/rgst/rgstiqbt')}", 'id' => session('userId'), 'mobile' => session('user.mobile')]));
            }*/
            //think\Log::notice(json_encode($data));
            /*if (session("user.status") == 6018005) {
                return json(array('code' => '4', 'msg' => '用户审核中', 'data' => ['url' => "{:url('/user/rgst/rgstiqbt')}",'id' => session('userId'), 'mobile' => session('user.mobile')]));
            }*/
            $menus=[];
            $msg=findById("userRole",array("id"=>$roleId),"menuIds");
            if($msg["code"]==='1'){
                $menuIds=$msg["data"]["menuIds"];
                $rlt=getDataList("userMenu",array("id"=>['in',$menuIds]),"name as  title,url,icon,id,flag,level,parentId","sort desc");


                if(!empty($rlt["data"])){
                    for ($s = 0; $s < count($rlt["data"]); $s++) {
                        $icon=$rlt["data"][$s]["icon"];
                        if(strpos($icon, "fa-") === 0){
                            $rlt["data"][$s]["icon"]="fa ".$icon;
                        }else{
                            $rlt["data"][$s]["icon"]="icon iconfont icon-".$icon;
                        }
                        if($rlt["data"][$s]["level"]==1){;
                            $rlt["data"][$s]["children"]=[];
                            $menus[]=$rlt["data"][$s];
                        }
                    }
                    for ($i = 0; $i < count($rlt["data"]); $i++) {
                        if($rlt["data"][$i]["level"]==2){
                            for ($j = 0; $j < count($menus); $j++) {
                                if($menus[$j]["id"]==$rlt["data"][$i]["parentId"]){
                                    $rlt["data"][$i]["url"]=url($rlt["data"][$i]["url"]);
                                    $menus[$j]["children"][]=$rlt["data"][$i];
                                }
                            }
                        }
                    }
...
//                        array("url" => url('/work/Work/index'), 'icon' => "gongzuotai", 'title' => '工作台', 'flag' => 'work'),
...
//                        array("url" => url('/iqbt/Iqbt/serview'), 'icon' => "service", 'title' => '申请服务', 'flag' => 'service'),
//                        array("url" => url('/user/user/iqbtpublic'), 'icon' => "remind", 'title' => '公告管理', 'flag' => 'remind'),
//                    );
//                    break;
//                case "6019002":
//                    /*return array(
...
//                    } else {
//                        return array(
//                            array("url" => url('/user/user/bhd-index'), 'icon' => "tijikongjian-xianxing", 'title' => '首页', 'flag' => "first"),
//                            array("url" => url('/iqbt/Iqbt/myiqbt'), 'icon' => "tijikongjian-xianxing", 'title' => '我的入驻', 'flag' => "space"),
//                            array("url" => url('/activity/Activity/etprsactlist'), 'icon' => "tubiao-", 'title' => '活动报名', 'flag' => "activity"),
//                            array("url" => url('/meetroom/Meetroom/mtroometprslist'), 'icon' => "huiyishi", 'title' => '会议室', 'flag' => 'mtroom'),
//                        );
...
//                case "6019003":
//                    return array(
//                        array("url" => url('/iqbt/Iqbt/adminindex'), 'icon' => "tijikongjian-xianxing", 'title' => '空间管理', 'flag' => 'space'),
...
//                    return array(
//                        array("url" => url('/iqbt/Iqbt/adminindex'), 'icon' => "tijikongjian-xianxing", 'title' => '空间管理', 'flag' => 'space'),
//                        array("url" => url('/user/user/auditlist'), 'icon' => "kehuguanli", 'title' => '用户管理', 'flag' => 'audit'),
//                        array("url" => url('/user/user/publicsay'), 'icon' => "remind", 'title' => '公告管理', 'flag' => 'remind'),
    ...
    public function dict($table)
    {
        ...
        switch($table){
            ...
            case "Incubator":
                ...
                $data["status"]=array('sysDict','code,name',self::setDictCon("6026"));
                $data["from"]=array('sysDict','code,name',self::setDictCon("6025"));
