...
(function () {
    ...
    App.extend({
        run: function (next) {
            ...
            $content.on('click', '[data-tag="list-delete"]', function (e) { // 删除顶部菜单
                ...
                alertify.theme('bootstrap')
                    .confirm("您确定要删除该菜单吗？", function () {
                        $.ajax({
                            url: domainUrl + '/deletedMenu?id=' + ID,
                            type: 'POST',
                            dataType: 'JSON',
            ...
            $content.on('focusout', 'input[name="submenu_sort"]', function () {
                var thisVal = $(this).val(),
                    $item = $nestable.find('.active').parent('li'),
                    storeVal = $item.data('url');
                ...
                if (thisVal === "") {
                    ...
                    self.subType = 'change';
                    $item.data("sort", thisVal).attr("data-sort", thisVal);
            ...
            $content.on('focusout', 'input[name="submenu_desc"]', function () {
                var thisVal = $(this).val(),
                    $item = $nestable.find('.active').parent('li'),
                    storeVal = $item.data('url');
                ...
                if (thisVal === "") {
                    ...
                    self.subType = 'change';
                    $item.data("desc", thisVal).attr("data-desc", thisVal);
        ...
        saveData: function () {
            ...
            var save = [];
            save.push(this.menuObj);
            $.ajax({
                url: domainUrl + '/saveAllMenu',
                type: 'POST',
                data: {menu: JSON.stringify(save)},
                dataType: 'JSON',
                success: function (data) {
                    if (data.success === true) {
                        self.subType = undefined;
                        toastr.info("菜单数据保存成功！");
                        // location.reload();
                    } else {
                        toastr.error(data.msg);
                    }
        ...
        getAuth: function (ID, type) {
            ...
            $.ajax({
                url: domainUrl + '/getTop?isInsert=1',
                data: {menuId: ID},
                dataType: 'JSON',
                success: function (data) {
                    var html = '', selected,
        ...
        submenuEdit: function (menuData) {
            ...
            $('.icp-dd1').iconpicker($.po('iconpicker', {title: '请选择菜单图标'}))
                ...
                // $('.icp-dd1').iconpicker($.po('iconpickerWb')).on('iconpickerSelected', function (e) {
                //     $(this).prev('span').children('i[data-icon]').data('icon', e.iconpickerValue).attr('data-icon', e.iconpickerValue);
                // });
        },
            ...
            var callback = function (icon) {
                ...
                if (icon) {
                    ...
                    html = '<li class="dd-item dd-item-alt" data-id="" data-auth="" data-rank="' + dataRrank +
                        '" new-build="' + newMenu + '" data-text="自定义菜单" data-url="#" data-icon="fa-bars" data-type="add">' +
                ...
                .on('success.form.fv', function (e) {
                    ...
                    var validator = $(e.target).data('formValidation'),
                        ...
                        menuData = {
                            ...
                            name: $(e.target).find("input[name='submenu_name']").val()
                        };
