...
function infor(res, a) {
    ...
    if (a == 1) {
        ...
        $('.tim-start').val(res.etprsInfo.pact_start)
    ...
    for (key in res.floor) { //楼层循环
        ...
        if (room != undefined) {
            for (num in room) {//办公室循环
                ...
                // 房间图标
                var boxg = $("<i class='icon wb-home font-size-40' aria-hidden='true' value = " + room[num].buildName + "></i>")
                //房间编号
        ...
        if (roomNo != undefined) {
            for (number in roomNo) {//工位循环
                ...
                //工位图标
                var boxk = $("<i class='icon wb-desktop font-size-30' aria-hidden='true' value = " + roomNo[number].buildName + "></i>")
                //工位编号
    ...
    $(function () {
        ...
        $(".colligation").on("click", "i", function (event) {
            ...
            var y = target.attr('value')
            $('.roomget').tokenfield('createToken', y + ' - ' + h + '㎡')
            $('.tim-start').val()
            //点击右侧显示合同开始时间与结束时间
    ...
    $('.change').on('click', function () {
        ...
        var arraya = [];
        for (sel in res.etprsInfo.roomInfo) {
            arraya.push(res.etprsInfo.roomInfo[sel].string.slice(res.etprsInfo.roomInfo[sel].string.search(/\d/g), res.etprsInfo.roomInfo[sel].string.indexOf(':')))
            for (arr in strArr) {
                // 数组的每一项值
                // 截取从第一位到':'位
                array.push(strArr[arr].slice(strArr[arr].search(/\d/g), strArr[arr].indexOf(':')))
                $('.icon-title').each(function () {
                    if (array[arr] == $(this).html()) {
                        ...
                        for (arrayb in arraya) {
                            if (roomInfo == '') {
                                roomInfo = res.etprsInfo.roomInfo[sel].id;
                            } else {
                                if (array[arr] == arraya[arrayb]) {
                                    roomInfo = roomInfo + ',' + res.etprsInfo.roomInfo[sel].id;
                                }
                            }
                        }
                    }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res["code"] == '1') {
                        ...
                        setTimeout(function () {
                            location.href = '/etprs/Etprs/etprsList';
                        }, 500);
                    } else {
